 ...
 * 
 * @ejb.bean name="PURMI07_PDF" description="An EJB named PURMI07_PDF"
 *           display-name="PURMI07_PDF" jndi-name="PURMI07_PDF" type="Stateless"
...
public abstract class PURMI07_PDFBean implements javax.ejb.SessionBean {
	...
	public static String reportTemplateUrl = System
			...
			+ "\\server\\default\\deploy\\zReportUtil.war\\report\\iPURMI07\\iPURMI07.jrxml";
	...
	private NaNaLog getLog() {
		if (this.log == null) {
			this.log = NaNaLogFactory.getLog(PURMI07_PDFBean.class);
	...
	public void createPDF(FormInstance tFormInstance) {
		try {
			...
			//載入語系定義檔
			String langName = "zh_TW";
			
			//判斷語系參數
			if(lang.equals("2")){
				langName = "en_US";
			}			
			String bundlePath = System.getProperty("jboss.home.dir")+ "\\server\\default\\deploy\\zReportUtil.war\\report\\iPURMI07\\ResBundle\\myRes_"
			 + langName + ".properties";
			...
			File jasperFile = new File(
					System.getProperty("jboss.home.dir")
							+ "\\server\\default\\deploy\\zReportUtil.war\\report\\iPURMI07\\iPURMI07.jasper");
	...
	public static List findReportData(FormInstance tFormInstance) {
		...
		tForm.setC_SchValue(tFormInstance.fetchFieldValue("txt_SchValue")); //學生期選項
		...
		// [column footer] -- 此處已移至TABLE
		/*
		tForm.setTC009(tFormInstance.fetchFieldValue("TC009")); // 備註
		tForm.setTC023(numFmt.format(Double.parseDouble(tFormInstance.fetchFieldValue("TC023")))); // 數量合計(設置千分位)
		tForm.setTC019(numFmt.format(Double.parseDouble(tFormInstance.fetchFieldValue("TC019")))); // 金額合計(設置千分位)
		tForm.setTC020(numFmt.format(Double.parseDouble(tFormInstance.fetchFieldValue("TC020")))); // 稅額(設置千分位)
		tForm.setTC019C1(numFmt.format(Double.parseDouble(tFormInstance.fetchFieldValue("TC019C1")))); // 總計(設置千分位)
		tForm.setC_TaxType(tFormInstance.fetchFieldValue("txt_TaxType")); // 課稅別(TC018C)

		tForm.setC_TotalinChinese(tFormInstance
		...
		*/
		...
		/* 20180517-改為帶預設圖, 除了1525501/2002054001 */
		/* 20181218-改為Table顯示 */
		/* 嘜頭List */
	...
	private static String checkImage(String imgUrl, String custID, String lastFileName, String markDesc){
		String frontFileName = "http://ref.prokits.com.tw/ERP_Files/Cust_Mark/Standard/";
		String fullFileName = "";
		
		if(custID.equals("1525501")){
			if(imgUrl.equals("")){
